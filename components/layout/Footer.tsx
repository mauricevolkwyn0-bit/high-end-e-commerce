...
export function Footer() {
  return (
    <footer className="bg-obsidian-dark border-t border-obsidian-border mt-section">
      {/* Newsletter strip */}
      <div className="border-b border-obsidian-border px-6 lg:px-12 py-14 max-w-screen-2xl mx-auto">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
          <div>
            <p className="text-2xs tracking-[0.3em] uppercase text-obsidian-gold mb-2">
              Private Access
            </p>
            <h3 className="font-display text-3xl lg:text-4xl text-obsidian-cream">
              Receive the exceptional.
            </h3>
          </div>
          <form className="flex gap-0 w-full lg:w-auto lg:min-w-[400px]">
            <input
              type="email"
              placeholder="Your email address"
              className="flex-1 bg-transparent border border-obsidian-border border-r-0 px-5 py-3.5 text-sm text-obsidian-cream placeholder-obsidian-muted focus:outline-none focus:border-obsidian-gold transition-colors"
            />
            <button
              type="submit"
              className="bg-obsidian-gold text-obsidian-black px-6 py-3.5 text-2xs tracking-[0.2em] uppercase font-medium hover:bg-obsidian-gold-light transition-colors duration-300 whitespace-nowrap"
            >
              Subscribe
            </button>
          </form>
      ...
      {/* Bottom bar */}
      <div className="border-t border-obsidian-border px-6 lg:px-12 py-6 max-w-screen-2xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
